...
#let figure-2 = canvas({
  ...
  // Scale and translate functions: a labeled circle and its conditioning triangle.
  for (name, pos, baseline, color, transparency) in (
    ("t", (4.3 * horiz-sep, 0.4 * -vert-sep), 0pt, teal, 40%),
    ("s", (rel: (.6, -.75), to: "t-circle"), -1pt, orange, 30%),
  ) {
    content(
      pos,
      text(fill: white, baseline: baseline, name),
      frame: "circle",
      name: name + "-circle",
      stroke: none,
      fill: color,
  ...
  for line-name in ("line-d-plus-1", "lineD") {
    for (source, operation, color) in (("s", "odot", orange), ("t", "oplus", teal)) {
      hobby(
        source + "-circle",
        line-name + "-" + operation,
