...
#let peak-overlay-threshold = 1.6
...
#canvas({
  ...
  let draw-surface-pass(draw-peak-overlay) = {
    for y-idx in range(surface-subdivisions) {
      ...
      for x-idx in range(surface-subdivisions) {
        ...
        let should-draw-quad = if draw-peak-overlay {
          z-avg >= peak-overlay-threshold
        } else { z-avg < peak-overlay-threshold }
        if should-draw-quad {
          line(
  ...
  draw-surface-pass(false)
  draw-surface-pass(true)
  ...
  content(
    ...
    text(size: axis-label-font-size)[$n_B(p_0)$],
    anchor: "west",
  )
})
